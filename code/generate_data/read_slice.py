...
class Paraser(object):
    def __init__(self) -> None:
        ...
        self.parser.add_argument('--data_root',     default='./original/slice/', help='data to be processed from EDA tools')
        self.parser.add_argument('--lef_path',      default='./original/tech2_lef/', help='path to LEF files')
        self.parser.add_argument('--def_path',      default='./original/slice/def/', help='path to DEF files')
        self.parser.add_argument('--rpt_path',      default='./original/slice/pwr_rpt/', help='path to power rpt files')
        self.parser.add_argument('--bumpfile_path', default='./original/slice/bumpfile/', help='path to bump locations files')
        self.parser.add_argument('--iv_path',       default='./original/slice/iv/', help='path to instance voltage files')
        
        self.parser.add_argument('--save_path',     default='./processed/training_set', help='processed data input into the network')

        self.parser.add_argument('--unit', default=1000, help='set 1000 when design is slice')
        ...
        for line in read_file:
            ...
            if READ_MACRO:
                if line.lstrip().startswith('SIZE'):
                    ...
                    lef_dict[cell_name]['size'] = [float(l[0]), float(l[1])] # size [unit*w,unit*h]
...
class ReadInnovusOutput:
        ...
        with open(self.route_def_path, 'r') as f:
            for line in f:
                ...
                if READ:
                    ...
                    if coords[1][0] == '*':  # vertical stripe
                        x1 = float(x1 - width/2)
                        x2 = float(x1 + width/2)
                        y2 = float(h / self.unit)
                    elif coords[1][1] == '*':  # horizontal stripe
                        y1 = float(y1 - width/2)
    ...
    def apply_tsv_mask(self):
        ...
        pattern = re.compile(r"rdl", re.IGNORECASE) 
    ...
    def compute_pdn_coverage_map(self, radius=3, sigma=1.2):
        ...
        return coverage_map[None, :, :]  # shape: [1, H, W]


    def compute_pdn_maps(self):
        
        W = self.patch_size[0]
        H = self.patch_size[1]

        pdn_map = np.zeros((H, W))
        ...
        for net_type in ["VDD", "VSS"]:
            for layer, segments in self.pdn_dict.get(net_type, {}).items():
                for x1, y1, x2, y2 in segments:
                    ...
                    x_min_, x_max_ = sorted([px1, px2])
                    y_min_, y_max_ = sorted([py1, py2])
                    pdn_map[y_min_:y_max_+1, x_min_:x_max_+1] += 1

        pdn_map = pdn_map / np.max(pdn_map) if np.max(pdn_map) > 0 else pdn_map
        return pdn_map
    ...
    def patch_based_feature_map(self, tsv_map, hb_map):
        ...
        if 'bottom_die' in self.save_name:
            core_x1 = (self.die_area[0] - 1228.3) / 2
            core_x2 = self.die_area[0] - core_x1
            core_y1 = (self.die_area[1] - 953.3) / 2
            core_y2 = self.die_area[1] - core_y1
            core_cx1 = int(core_x1 / self.step_x)
            core_cx2 = int(core_x2 / self.step_x)
            core_cy1 = int(core_y1 / self.step_y)
            core_cy2 = int(core_y2 / self.step_y)
            bot_mask[core_cy1:core_cy2+1, core_cx1:core_cx2+1] = 1.0
            for ch in range(feature_map.shape[0]):
                feature_map *= bot_mask
